...
public class TestStopInScript {
    ...
    public void test(String engineName) throws Exception {
        ...
        Assert.assertTrue("The script exception must contain the message of stop() function",
                          res.getException().getMessage().contains(errorMessage));
        Assert.assertTrue("The script output must contain the stop message", output.toString().contains(errorMessage));
        Assert.assertEquals("Even if the script is stoppped with stop() all changes in the variables map must be done",
